...
class Application(QWidget):
    ...
    def start_resizing(self):
        ...
        self.progress.setMaximum(self.total_images)  
        self.thread = ResizeThread(self.TARGET_SIZE, self.total_images)  
    ...
    def save_log(self):
        try:
            ...
            archive_path = os.path.join(log_folder, 'Trim Image.7z')
            ...
            if not os.path.exists(archive_path):
                with py7zr.SevenZipFile(archive_path, mode='w', password='@Joe11111111') as archive:
                    archive.writestr(new_log_message, f'Trim Image/{log_filename}')
            else:
                log_content = ""
                files_to_keep = []
                ...
                with py7zr.SevenZipFile(archive_path, mode='r', password='@Joe11111111') as archive:
                    for filename, bio in archive.read().items():
                        if filename == f'Trim Image/{log_filename}':
                ...
                with py7zr.SevenZipFile(temp_archive_path, mode='w', password='@Joe11111111') as archive:
                    archive.writestr(log_content.encode('utf-8'), f'Trim Image/{log_filename}')
